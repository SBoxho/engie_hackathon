...
def consumption_chart(frame: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(xaxis_title=None, yaxis_title="MW", hovermode="x unified")
    return fig
...
def production_area_chart(frame: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(xaxis_title=None, yaxis_title="MW", hovermode="x unified", legend_title=None)
    return fig
...
def mix_donut(row: pd.Series) -> go.Figure:
    ...
    fig.update_layout(showlegend=True, legend_title=None, margin=dict(l=10, r=10, t=20, b=10))
    return fig
